...
use super::{InterruptType, Timer, MANAGER, TIMERS};
use core::arch::asm;
...
pub unsafe fn setup() {
    ...
    let mut timer_lvt = TimerLvt::from_u32(local_apic.read_register(LocalApicRegister::LvtTimer));
...
unsafe fn countdown_ended() -> bool {
    (*tls::get()).local_apic.interrupt_received
}
