...
extern "C" {
    fn init64(kernel_args_ptr: core::ptr::NonNull<kernel_args::Args>) -> !;
}
    ...
    // Generate kernel memory bitmap, initialise page allocator
    {
        ...
        let kernel_bitmap = memory_map
            ...
            .find_map(|entry| match entry.entry_type {
                super::MemoryMapEntryType::Usable if entry.length >= memory_map_page_size => Some(
                    core::slice::from_raw_parts_mut(entry.base as *mut u8, memory_map_page_size),
                ),
                _ => None,
            })
            .unwrap_or_else(|| {
